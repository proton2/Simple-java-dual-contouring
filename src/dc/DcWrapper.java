...
public class DcWrapper extends GameObject {
    ...
    private void renderMesh(){
        //root = Octree.BuildOctree(new Vec3f(-octreeSize / 2, -octreeSize / 2, -octreeSize / 2), octreeSize, THRESHOLDS[thresholdIndex]);
        root = Octree.BuildOctree(updateFrustum,
                getTransform().getProjectionMatrix(), getTransform().getViewMatrix(),
                new Vec3f(-octreeSize / 2, -octreeSize / 2, -octreeSize / 2), octreeSize, THRESHOLDS[thresholdIndex]);
    ...
    private void testPlane() {
        ...
        renderer = new Renderer(meshBuffer);
        renderer.setRenderInfo(new RenderInfo(new CCW(), DcSimpleShader.getInstance()));
        addComponent(Constants.RENDERER_COMPONENT, renderer);
    }
